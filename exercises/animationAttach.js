...
function createSphere() {
    let sphGeo = new THREE.SphereGeometry(1, 20, 20);
    let newSphere = new THREE.Mesh(sphGeo, material2);
    newSphere.position.set(0, 0, 3); // mesma posição da esfera original em relação ao cubo
    cube.add(newSphere);
    return newSphere;
}
...
function render() {
    ...
    // Mover todas as esferas disparadas para frente
    shotSpheres.forEach(s => {
        s.translateZ(0.2); // velocidade do disparo
    });
...
function keyboardUpdate() {
    ...
    if (keyboard.down("space")) {
        let newShot = createSphere();
        shotSpheres.push(newShot);
    }
...
function updatePositionMessage() {
    if (shotSpheres.length === 0) return;

    let lastSphere = shotSpheres[shotSpheres.length - 1];
    let wp = new THREE.Vector3();
    lastSphere.getWorldPosition(wp);

    var str = "Última Esfera - Local {" + lastSphere.position.x.toFixed(1) + ", " + lastSphere.position.y.toFixed(1) + ", " + lastSphere.position.z.toFixed(1) + "} " +
